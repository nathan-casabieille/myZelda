...
{
    sfFloatRect adjusted_bounds = get_adjusted_bounds(camera, PLAYER_WIDTH, PLAYER_HEIGHT);

    sfVector2f player_position = player->position;

    if (player_position.x <= adjusted_bounds.left || 
        player_position.x >= adjusted_bounds.left + adjusted_bounds.width ||
        player_position.y <= adjusted_bounds.top ||
        player_position.y >= adjusted_bounds.top + adjusted_bounds.height) {
        return sfTrue;
    }
